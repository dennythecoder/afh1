...
mb.store = new Vue({
    ...
    methods:{
		...
		searchPages:function(searchTerm){
			...
			for(var i = 0; i < this.pages.length; i++){
				...
				if(page && page.content && page.content.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1){
					result.push(page);
				}
			}
			return result;
    ...
    created:function(){
        ...
        if(jsonBookmarks){Vue.set(this,'bookmarks',JSON.parse(jsonBookmarks))}
        const jsonLastLocation = localStorage.getItem('lastLocation');
        if(jsonLastLocation) { Vue.set(this,'lastLocation', JSON.parse(jsonLastLocation));}
